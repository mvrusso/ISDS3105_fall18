...
invoke_map(list(avg = mean, IIQ = median), list(x = bad_drivers$insurance_premiums, x = bad_drivers$insurance_premiums))
...
drawRoom <- function(data, title) {
  ggplot(data = data)+
    ...
    ggtitle('Average Room Rates for', title)
}

group_performance1 <- performance %>% group_by(hotel) %>% nest()

group_performance2 <- group_performance1 %>% mutate(rates = pmap(list(data = .$data, title = .$hotel), .f = drawRoom))

drawRoom(group_performance1$data[[1]], 'Tapei') #This is just an example to see how the function works.

ggsave('timeLines', plot = last_plot())
```
