...
bad_driversNested <- bad_driversNested %>% mutate(stateMap = map2(.x = .$stateShape, .y = .$isSouth, drawMap))
...
Create a ggplot function to draw a linechart of the `AverageRoomRate` over time (`Month`) and save it into a folder 'timeLines'. Use iteration to create a separate chart of each hotel and save it. Make sure each chart is titled and saved meaningfully.
...
drawRoom <- function(data, title) {
  ggplot(data = data)+
    geom_line(aes(x = Month, y = AverageRoomRate))+
    ggtitle(paste('Average Room Rates for', title))
}

group_performance1 <- performance %>% group_by(hotel) %>% nest()

x <- c(1:length(group_performance1$data))

for(i in seq_along(x)) {
  ggsave(filename = paste0(group_performance1$hotel[i], '.png'),
         plot = drawRoom(group_performance1$data[[i]], group_performance1$hotel[i]), path = here('lectures', 'lesson13_purrr_2', 'timeLines'))
}
