...
dt_crimes1 <- read_rds(here::here('lectures', 'lesson15_regression', 'data', 'crimes.rds'))
#merge the two dataset
dt_crimes2 <- dt_crimes1 %>% left_join(dt_temp, by = c('offense_date' = 'Day'))
```
...
```{r visualizing a lm}
ggplot(data = dt_crimes2, aes(x = TempHigh, y = n)) +
  geom_point() +
...
## What can I use a glm model for?

- *Explanation*: you can write statements such as:

There is a significant positive association between heat and crimes. In particular, for a 1 unit increase in the predictor (TempHigh), there is a `r exp(fit$coefficients['TempHigh'])` increase in the count of crimes.
...
Checking the model fit means assessing how good our model is in reflecting our actual outcome variable. That means analyzing the residuals looking for patterns that suggest misfit. What follows are some visual tools to analyze residuals. This list is *very far* from being complete and exhaustive. However, it shold give you an idea of looking only at significance can be very misleading when interpreting the results of a regression model.
...
How to remedy for poor fit?
 ...
 - Use different glm, maybe the relationship is non-linear (but that's beyond our scope for this class)
 

What does it mean when we add another predictor? Suppose we add precipitations too

```{r}
fit2 <- glm(n ~ TempHigh + Precip, family = poisson(link = 'log'), data = dt_crimes2)
```
...
# This won't render in Markdown (need html):
 library(plotly)
 plot_ly(data = dt_crimes2, z = ~n, x = ~TempHigh, y = ~Precip,opacity = 0.5) %>%
