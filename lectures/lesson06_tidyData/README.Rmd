...
gather(untidyReview, key = 'review', value = 'reviewValue', -reviewId, -reviewStayYear, na.rm = TRUE) %>%
  ggplot()+
  geom_bar(aes(x = reviewValue))+
  facet_wrap(~ review)

gather(untidyReview, key = 'Attribute', value = 'rating', reviewOverall:reviewValue) %>% 
  ggplot()+
...
ggplot()+
geom_line(aes(x = Quarter, y = Sales, color = factor(year), group = year))
...
ggplot(data = socialNetwork_users)+
...
gather(data = iphoneSales, key = 'PhoneType', value = 'Sales', -time, na.rm = TRUE) %>%
ggplot() +
geom_bar(aes(x = PhoneType, y = Sales), stat = 'identity')

  
gather(iphoneSales, key = 'Model', value = 'Sales', -time, na.rm = TRUE)%>% 
  ggplot()+
  geom_col(aes(x = Model, y = Sales))
```
