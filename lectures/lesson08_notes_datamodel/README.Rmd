...
```{r}
x <- tbl(con, 'review')
# do the pipe then collect if you want it on your laptop
tbl(con, 'review') %>% mutate(mean(reviewService, na.rm = T))
#Some functions in R, like mean, are not available in SQL and vice versa.
```
...
#When you collect, you take everything you're querying from the server and transforming it into a tibble. 
```
...
#This second thing is more for SQL people.
```
...
#The part about rename(total = n) was added to rename the column.  It is optional.  
...
```{r}
tbl(con, 'hotel') %>% inner_join(tbl(con, 'review'), by = c('hotelId' = 'hotelId')) %>% group_by(hotelName) %>% collect() %>% summarise(avg = mean(reviewService)) %>% arrange(desc(avg)) %>% top_n(3, avg) 

tbl(con, 'hotel') %>% inner_join(tbl(con, 'review'), by = c('hotelId' = 'hotelId')) %>% group_by(hotelName) %>% summarise(avg = mean(reviewService)) %>% arrange(desc(avg)) %>% collect() %>% slice(1:3)
```
...
```{r}
tbl(con, 'hotel') %>% inner_join(tbl(con, 'review'), by = 'hotelId') %>% inner_join(tbl(con, 'author'), by = 'authorId') %>% select(hotelId, hotelName, authorNickname) %>% arrange(authorNickname)
...
```{r}
hotel_joins_review <- tbl(con, 'hotel') %>% inner_join(tbl(con, 'review')) %>% collect() %>%  gather(key = 'review_star', value = 'review_starValue', reviewHelpfulness:reviewText) 

tbl(con, 'hotel') %>% inner_join(tbl(con, 'review')) %>% collect() %>%  gather(key = 'review_star', value = 'review_starValue', reviewHelpfulness:reviewText, na.rm = TRUE)%>% count(hotelId, hotelName)
...
tbl(con, 'hotel') %>%
left_join(tbl(con, 'review')) %>% filter(is.na(reviewLocation)) %>% select(hotelId, hotelName, reviewLocation)
 

group_by(hotelId, hotelName) %>%
 summarise(tot = n()) %>%
 select(hotelName, tot)
```
