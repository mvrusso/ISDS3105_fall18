...
Using `mean()` on the column-list doesn't work: 
...
for (i in seq_along(x)) {            # sequence
  output[i] <- typeof(x[[i]])        # body; iterating over each element of list; using just [ ] gives a sublist
}
...
```{r}
x <- list(1, 'hi', TRUE, T) 
map(.x = x, .f = typeof) #originally just had this
purrr::map(.x = x, .f = typeof) #with this notation you never go wrong
```
...
x <- rep(x, 50)
microbenchmark(
  withMap = { map(x, typeof) }, #can also use purrr::map
  withLoop = {
...
purrr::map(.x=select(iphoneSales, -year), .f=~mean(.x, na.rm=T))
```
...
t_prime <- t %>% mutate(nums = map(.x = nums, 2, .default = as.character(NA)))
#' we can simplify the data structure to an atomic vector using 
#' the functions map_chr, map_int, map_dbl, map_lgl
t_prime_chr <- t %>% mutate(nums = map_chr(.x = nums, 2, .default = as.character(NA)))

#using map() to pluck values like iterating the subset function `[`
t_pluck <- t %>% mutate(nums = map_chr(.x = nums, `[`, 2))
```

You can extract also by element name:

```{r}
t_elem_name <- t %>% mutate(nums = map_chr(.x = nums, 'b', .default = NA))
```
...
bad_driversSouth_split <- bad_driversSouth %>% 
  split(.$isSouth) %>% 
...
bad_driversMountains <-  bad_drivers %>% mutate(US_region = case_when(state %in% c('Louisiana', 'Alabama', 'Mississippi', 'Georgia', 'South Carolina') ~ 'South', state %in% c('Montana', 'Wyoming', 'Colorado', 'New Mexico', 'Idaho', 'Utah', 'Arizona', 'Nevada') ~ 'Mountain', T ~ 'Other'), state = tolower(state))

#First Solution
bad_driversMountains %>% 
  split(.$US_region) %>% 
  purrr::map(~ right_join(.x, states, by = c('state' = 'region')) %>% 
                      filter(!is.na(insurance_premiums)) %>% 
                      ggplot() +
...
#Second Solution
bad_drivers_Nested <- bad_driversMountains %>% group_by(US_region) %>% nest()

bad_drivers_Nested <- bad_drivers_Nested %>% 
 mutate(insurance_map = purrr::map(.x = data, ~ right_join(.x, states, by = c('state' = 'region')) %>% 
                      filter(!is.na(insurance_premiums)) %>% 
                      ggplot() +
                      geom_polygon(aes(x = long, y = lat, group = group, fill = insurance_premiums), color = "white") + 
                      coord_fixed(1.3) +
                      labs(title = paste('Insurance premiums'))))
bad_drivers_Nested$insurance_map[1]
bad_drivers_Nested$insurance_map[2]
bad_drivers_Nested$insurance_map[3]
```
