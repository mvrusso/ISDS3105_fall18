...
```{r}
untidyReview %>% 
  filter(is.na(reviewLocation) == TRUE)
```
...
```{r}
untidyReview %>% select(reviewOverall:reviewValue)

untidyReview %>% select(-c(reviewId, reviewStayYear))
```
...
```{r}
untidyReview %>% mutate(Year = as.integer(reviewStayYear))
```
...
```{r}
titanicDt %>% filter(Survived == 'No') %>% group_by(Class) %>% summarise(avg_class = mean(n))
```
...
```{r}
titanicDt %>% filter(Survived == 'No') %>% group_by(Sex) %>% summarise(avg_gender = mean(n))
```

3. Using `beerDt`, calculate the average consumption before and after your birthyear (hint: you may create a new variable)
```{r}
beerDt$preMBY <- beerDt$Year < 1985
beerDt %>% group_by(preMBY) %>% summarise(preAVG = mean(GallonsCapita))

beerDt$postMBY <- beerDt$Year >1985
beerDt %>%  group_by(postMBY) %>% summarise(postAVG = mean(GallonsCapita))
```

4. Using `selfiesCasualties`, calulcate the count of casualties by nationality, then arrange descending.
```{r}
selfiesCasualties %>% group_by(nationality) %>% summarise(count_nationality = n()) %>% arrange(desc(count_nationality))
```
...
selfiesCasualties %>% count(country, gender)
```
...
```{r}
selfiesCasualties %>% group_by(class) %>% summarise(percent_class = n()/85*100)
```
...
```{r}
add_carrier_name <- left_join(flights, airlines, by = 'carrier') 
```
...
```{r}
add_carrier_name %>% group_by(name) %>% summarise(tot = n()) %>% ggplot()+
  geom_col(aes(x = name, y = tot))
...
```{r}
add_carrier_name %>% group_by(origin, dest) %>% summarise(busy_route = n()) %>% arrange(desc(busy_route))


add_carrier_name$route <- paste(add_carrier_name$origin, add_carrier_name$dest)
add_carrier_name %>%  
  group_by(route) %>%
  summarise(tot = n()) %>%
  arrange(desc(tot))
...
```{r}
add_carrier_name %>% group_by(year, month, day, origin) %>% summarise(count_day = n()) %>% arrange(desc(count_day))

add_carrier_name$date <- paste(add_carrier_name$month, add_carrier_name$day, add_carrier_name$year, sep = '/')
